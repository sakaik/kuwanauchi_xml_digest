...
#TODO math化/rad-deg変換はnpを使っているがここだけなら自前実装してもよさげ。（本スクリプトを動かすための前提条件をなるべく減らしたい）
...
def xy2do(x, y, japaneseKei):
    ...
    #戻り rad単位で原点座標の(lon, lat)
    # https://www.gsi.go.jp/LAW/heimencho.html
    # TODO: 原点資料を見ながら配列を作っていたら (lon,lat) になってしまった。本ファイルの中では(lat,lon)に統一したい。（(x,y)の順に対応するため）
    def getPhiLambFromKeiByRadian(kei):
        ary = [ (0,0), 
               (129.5,33.0),          (131.0,33.0),          (132+(10.0/60),36.0), (133.5,33.0),          (134+(20.0/60),36.0),
               (134+(20.0/60), 36.0), (137+(10.0/60),36.0),  (138.5, 36.0),        (139+(50.0/60), 36.0), (140+(50.0/60), 36.0),
               (140+(15.0/60), 44.0), (142+(15.0/60), 44.0), (144+(15.0/60),44.0), (142.0, 26.0),         (127.5, 26.0),
               (124.0, 26.0),         (131.0, 26.0),         (136.0, 20.0),        (154.0, 26.0)
               ]
    ...
    def makeAryBeta(n):
        ...
        return np.array([np.nan, beta1, beta2, beta3, beta4, beta5])
    ...
    def makeAryDelta(n):
        ...
        return np.array([np.nan, delta1, delta2, delta3, delta4, delta5, delta6])

    def calcPartOfSbar_phi0(aryA, phi0):
        res = 0.0
        for j in range(1, 5+1):
            res += aryA[j] * np.sin(2*j*phi0)
        return res

    def calcPartOfXidash(aryBeta, xi, eta):
        res = 0.0
        for j in range(1, 5+1):
            res += aryBeta[j]*np.sin(2*j*xi)*np.cosh(2*j*eta)
        return res

    def calcPartOfEtadash(aryBeta, xi, eta):
        res = 0.0
        for j in range(1, 5+1):
            res += aryBeta[j]*np.cos(2*j*xi)*np.sinh(2*j*eta)
        return res

    def calcPartOfPhi(aryDelta, chi):
        res = 0.0
        for j in range(1, 6+1):
            res += aryDelta[j]*np.sin(2*j*chi)
        return res


    #平面直角座標系原点座標取得
    (lambda0, phi0) = getPhiLambFromKeiByRadian(japaneseKei)
